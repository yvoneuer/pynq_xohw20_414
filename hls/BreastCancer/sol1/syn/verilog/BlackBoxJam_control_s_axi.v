...
localparam
    ...
    ADDR_VAL_V_DATA_0       = 7'h48,
    ADDR_VAL_V_DATA_1       = 7'h4c,
    ADDR_VAL_V_CTRL         = 7'h50,
    ADDR_FIX_VAL_V_DATA_0   = 7'h54,
    ADDR_FIX_VAL_V_CTRL     = 7'h58,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_FIX_VAL_V_DATA_0: begin
                    rdata <= int_fix_val_V[23:0];
                end
            endcase
        end
    end
...
assign fix_val_V   = int_fix_val_V;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_fix_val_V[23:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_FIX_VAL_V_DATA_0)
            int_fix_val_V[23:0] <= (WDATA[31:0] & wmask) | (int_fix_val_V[23:0] & ~wmask);
    end
end
